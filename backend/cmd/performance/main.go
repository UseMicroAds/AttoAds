...
import (
	...
	"github.com/microads/microads-backend/internal/performance"
...
func main() {
	...
	intervalMin, _ := strconv.Atoi(envOrDefault("PERFORMANCE_POLL_INTERVAL_MIN", "5"))
	...
	poller := performance.NewPoller(store, ytClient, pollInterval)
	go poller.Run(ctx)
	...
	slog.Info("performance worker shutting down")
	cancel()
}
